...
/* SPECS:
 *  1)  Optimized Polynomial Multiplication:
 *          a)  Elements are inserted in the heap in decreasing order, where the
 *              max element is selected from ALL the pairs. This is opposed to the 
 *              previous implementation, where a max was selected from EACH pair
 *              and inserted into the heap.
 *          b)  Direct merging is attempted in the buffer I when inserting elements.
 *              If an element is merged in I, then we can directly insert another
 *              element, while maintaining the size of the heap.
 *          c)  While sweeping, elements are merged together in order to save
 *              space in the heap.
 *  
 */
